...
const state = await getJson('/api/d1/read-model-state?v=60');
assert(state.dataLayerVersion === 'v0.6.0', `버전 불일치: ${state.dataLayerVersion}`);
...
const places = await getJson('/api/places?v=60');
assert(places.dataLayerVersion === 'v0.6.0', `places 버전 불일치: ${places.dataLayerVersion}`);
...
if (linked.length === 0) {
  console.warn('  WARN · 센텀 이름 검색은 되지만 EV 사전매칭 결과는 없습니다. match-debug 응답으로 원인을 확인하세요.');
}
