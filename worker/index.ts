...
type ParkingMatchRule = {
  parkingCode: string;
  parkingId: string;
  matchType: 'CODE_RULE' | 'ALIAS';
  confidence: number;
...
async function handlePlaces(env: Env, _ctx: ExecutionContext) {
  ...
  try {
    ...
    return json({ ok: false, dataLayerVersion: DATA_LAYER_VERSION, parkingMatchVersion: PARKING_MATCH_VERSION, error: safeError(error) }, 500);
  }
}
...
type ParkingRealtimeContribution = {
  parkingCode: string;
  parkingName: string;
...
type ParkingRealtimeSnapshotItem = {
  ...
  sourceUpdatedAt: string | null;
  fetchedAt?: string | null;
  components?: ParkingRealtimeContribution[];
};
...
async function ensureLiveSchema(db: D1Database) {
  await db.batch([
    ...
    db.prepare(`CREATE TABLE IF NOT EXISTS parking_match_rules (
      parking_code TEXT PRIMARY KEY,
    ...
    db.prepare(`CREATE INDEX IF NOT EXISTS idx_parking_match_rules_parking_id
...
async function loadParkingMatchRules(db: D1Database) {
  const rows = await db.prepare(
    `SELECT parking_code, parking_id, match_type, confidence, allow_aggregate
       FROM parking_match_rules
      ORDER BY parking_code`,
  ).all<{
    parking_code: string;
    parking_id: string;
    match_type: string;
    confidence: number;
    allow_aggregate: number;
  }>();

  return new Map<string, ParkingMatchRule>(
    rows.results.map((row) => [
      row.parking_code,
      {
        parkingCode: row.parking_code,
        parkingId: row.parking_id,
        matchType: row.match_type === 'ALIAS' ? 'ALIAS' : 'CODE_RULE',
        confidence: Number(row.confidence || 0),
        allowAggregate: Number(row.allow_aggregate || 0) === 1,
      },
    ]),
  );
}
...
function localParkingRealtimeFixture(): BusanRealtimeParkingApiItem[] {
  return [
    ...
    { parkgcd: 'A-CENTUM-2', parknm: '해운대센텀시티 제2구역', curravacnt: '4', parkingcnt: '6', maxcnt: '10', lastupdatetime: '2026-09-22 09:04:00' },
    ...
    { parkgcd: 'A-NEGATIVE', parknm: '음수테스트', curravacnt: '-1', parkingcnt: '11', maxcnt: '10', lastupdatetime: '2026-09-22 09:05:00' },
  ];
}
...
async function syncParkingRealtime(env: Env) {
  ...
  try {
    ...
    const matchRules = await loadParkingMatchRules(env.DB);
    const joined = joinRealtimeParkingByName(parkingBases, valid, matchRules);
    ...
    const contributionByParkingId = new Map<string, Map<string, ParkingRealtimeContribution>>();

    for (const [parkingId, snapshotItem] of existing.items) {
      const components = snapshotItem.components?.length
        ? snapshotItem.components
        : [{
            parkingCode: snapshotItem.parkingCode,
            parkingName: snapshotItem.parkingName,
            available: snapshotItem.available,
            occupied: snapshotItem.occupied,
            capacity: snapshotItem.capacity,
            sourceUpdatedAt: snapshotItem.sourceUpdatedAt,
            fetchedAt: snapshotItem.fetchedAt || existing.fetchedAt,
          }];
      contributionByParkingId.set(
        parkingId,
        new Map(components.filter((component) => component.parkingCode).map((component) => [component.parkingCode, component])),
      );
    }
    ...
    for (const item of valid) {
      ...
      if (!detail?.baseId || !code) continue;
      const components = contributionByParkingId.get(detail.baseId) || new Map<string, ParkingRealtimeContribution>();
      components.set(code, {
        parkingCode: code,
        parkingName: name,
      ...
      contributionByParkingId.set(detail.baseId, components);
      updatedCount += 1;
    }

    const snapshotByParkingId = new Map<string, ParkingRealtimeSnapshotItem>();
    for (const [parkingId, componentsMap] of contributionByParkingId) {
      const components = [...componentsMap.values()];
      ...
      const sourceTimes = components
        .map((component) => component.sourceUpdatedAt)
        .filter((value): value is string => Boolean(value))
        .sort((a, b) => parseSourceTimestampMs(a) - parseSourceTimestampMs(b));
      const fetchedTimes = components
        .map((component) => component.fetchedAt)
        .filter((value): value is string => Boolean(value))
        .sort((a, b) => Date.parse(a) - Date.parse(b));

      snapshotByParkingId.set(parkingId, {
        parkingId,
        parkingCode: components.map((component) => component.parkingCode).join(','),
        parkingName: components.map((component) => component.parkingName).join(' + '),
        available: sumNullable(components.map((component) => component.available)),
        occupied: sumNullable(components.map((component) => component.occupied)),
        capacity: sumNullable(components.map((component) => component.capacity)),
        sourceUpdatedAt: sourceTimes[0] || null,
        fetchedAt: fetchedTimes[0] || fetchedAt,
        components,
      });
    }
...
function joinRealtimeParkingByName(
  baseItems: ParkingBase[],
  realtimeItems: BusanRealtimeParkingApiItem[],
  rules: Map<string, ParkingMatchRule> = new Map(),
): ParkingJoinResult {
